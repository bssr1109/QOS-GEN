...
def main():
    ...
    # ---------- FINISH & DOWNLOAD ---------- ----------
    if not st.session_state.todo_tips:
        if st.button("Finish & Download PDF"):
            buffer = BytesIO()
            c = Canvas(buffer, pagesize=A4)
            pair_positions = [27, 15]
            for idx, cert in enumerate(st.session_state.get("certs", [])):
                draw_certificate(c, cert, pair_positions[idx % 2])
                if idx % 2 == 1:
                    c.showPage()
            if len(st.session_state.get("certs", [])) % 2 == 1:
                c.showPage()
            c.save()
